...
const names = [
  "Idumela Nelson", "Samuel Johnson", "Michael Brown", "Sarah Davis",
  "James Wilson", "Emma Thompson", "David Miller", "Lisa Anderson",
  "Robert Taylor", "Jennifer Martinez", "Christopher Lee", "Amanda Clark",
  "Matthew Rodriguez", "Ashley Lewis", "Daniel Walker", "Jessica Hall"
];
...
const CopyTradeManage = () => {
  ...
  const renderPaginationButtons = useCallback(() => {
    ...
    if (startPage > 1) {
      buttons.push(
        <button key={1} onClick={() => handlePageChange(1)} className="px-3 py-1 rounded-md hover:bg-gray-700 text-gray-300">
          1
        </button>
      );
      if (startPage > 2) {
    ...
    for (let i = startPage; i <= endPage; i++) {
      buttons.push(
        <button
          ...
          className={`px-3 py-1 rounded-md ${currentPage === i ? 'bg-gray-600 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
        >
          {i}
        </button>
      );
    ...
    if (endPage < totalPages) {
      ...
      buttons.push(
        <button key={totalPages} onClick={() => handlePageChange(totalPages)} className="px-3 py-1 rounded-md hover:bg-gray-700 text-gray-300">
          {totalPages}
        </button>
      );
    }
  ...
  const stats = useMemo(() => {
    ...
    const pausedUsers = filteredUsers.filter(user => user.status === 'Paused').length;
    const totalTrades = filteredUsers.reduce((sum, user) => sum + user.totalCopiedTrades, 0);
  ...
  // Pie chart data
  const pieData = [
    { name: 'Active', value: stats.activeUsers, color: '#00D4AA' },
    { name: 'Paused', value: stats.pausedUsers, color: '#FF6B6B' },
  ];
  ...
  const renderCustomizedLabel = ({
    ...
    const mx2 = mx + (cos >= 0 ? 1 : -1) * 10;
    const my2 = my;
    
    // End point (where the text starts)
    const ex = mx2 + (cos >= 0 ? 1 : -1) * 10;
    const ey = my2;
    ...
    <div className="min-h-screen  text-gray-100 p-8 font-inter">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-linear-to-l from-[#141E323D] to-[#01040F] rounded-lg p-6 border border-[#141E32]">
            <h3 className="text-sm text-gray-400 mb-2">Active Copy Trades</h3>
            <p className="text-2xl font-bold text-white">{stats.totalTrades.toLocaleString()}</p>
          </div>
          
          <div className="bg-linear-to-l from-[#141E323D] to-[#01040F] rounded-lg p-6 border border-[#141E32]">
            <h3 className="text-sm text-gray-400 mb-2">Pending Executions</h3>
            <p className="text-2xl font-bold text-white">{stats.activeUsers.toLocaleString()}</p>
          </div>

          <div className="bg-linear-to-l from-[#141E323D] to-[#01040F] rounded-lg p-6 border border-[#141E32]">
            <h3 className="text-sm text-gray-400 mb-2">User Status</h3>
        ...
        <h1 className="text-xl font-medium mb-6 text-gray-300">Users ({stats.totalUsers})</h1>

        {/* Search and Filter Section */}
        <div className="flex flex-col md:flex-row items-center justify-between  p-4 rounded-lg mb-6">
        ...
        <div className=" rounded-lg overflow-hidden">
          <table className="min-w-full">
            <thead className="bg-[#060A17]">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase whitespace-nowrap tracking-wider">User ID</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase whitespace-nowrap tracking-wider">Name</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase whitespace-nowrap tracking-wider">Email</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase whitespace-nowrap tracking-wider">Total Copied Trades</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase whitespace-nowrap tracking-wider">Profit/Loss</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase whitespace-nowrap tracking-wider">Status</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase whitespace-nowrap tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody >
              {currentUsers.length > 0 ? (
                currentUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-[#242938] transition-colors duration-200">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`${user.profitLoss.includes('-') ? 'text-red-400' : 'text-green-400'}`}>
                        {user.profitLoss}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
